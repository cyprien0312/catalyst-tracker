"""U100 / Nasdaq-100 buy-plan signal, keyed off the catalyst fuses.
...
# Regime A — shallow ladder, max 60% deployed, 40% reserve held back.
REGIME_A_LADDER = [
...
def build_plan(ndx: dict | None, fuse_lit: bool, fuses: list[str]) -> dict | None:
...
def render_html_card(plan: dict | None) -> str:
  ...
  <div style="background:#fff;border:1px solid #eef0f2;border-left:4px solid {col};border-radius:0 8px 8px 0;padding:14px 18px;margin:10px 0;">
    <div style="font-size:11px;font-weight:700;letter-spacing:1.2px;color:{col};">★ BUY PLAN · U100 / NDX</div>
...
def plan_from_rows(rows) -> dict | None:
    ...
    return build_plan(ndx_drawdown(), fuse_lit, fuses)
...
def live_plan() -> dict | None:
    ...
    return build_plan(ndx_drawdown(), bool(fuses), fuses)
